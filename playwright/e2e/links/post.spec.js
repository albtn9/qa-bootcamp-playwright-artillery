...
test.describe('POST /auth/links', () => {
    ...
    test('não deve encurtar quando a url original não é informada', async ({ links }) => {
     ...
     test('não deve encurtar quando a URL nao é válida', async ({ links }) => {
    
        const response = await links.createLink({...user.link, original_url:'teste@teste'}, token)
        expect(response.status()).toBe(400)
        const {message } = await response.json()
